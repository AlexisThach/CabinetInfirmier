...
function openFacture(prenom, nom, actes) {
    ...
    factureText = "Facture pour : " + prenom + " " + nom + "<br/>" + actes;
...
function adresseToText(adresse) {
    ...
    // Vérifier si l'élément "étage" existe
    var etageElement = adresse.getElementsByTagName("étage")[0];
    if (etageElement) {
        var etage = etageElement.childNodes[0].nodeValue;
        str += "Étage " + etage + ", ";
    }

    // Vérifier si l'élément "numéro" existe
    var numeroElement = adresse.getElementsByTagName("numéro")[0];
    if (numeroElement) {
        var numero = numeroElement.childNodes[0].nodeValue;
        str += "Numéro " + numero + ", ";
    }

    // Récupérer les autres composants de l'adresse
    ...
    // Assembler les composants dans une chaîne de texte
    str += "Rue " + rue + ", Ville " + ville + ", Code Postal " + codePostal;
